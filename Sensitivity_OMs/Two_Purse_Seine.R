...
# ===========================================================================================================================
# === Sensitivity scenario 18 - a two-purse fleet model =====================================================================
# ===========================================================================================================================
#
...
Cat<-readRDS(file=paste0(datdir,"Chist_3_200228.rds"))
...
data$Chist[,5]<-data$Chist[,5]/1000 # weir catches reduced to very low levels

# --- Historical equilibrium catch (30 years)

HCat0<-readRDS(file=paste0(datdir,"Chist_1_Inital_cond_200303.rds"))
prop_juv <- sum(Cat$PS_juv[1:10])/ (sum(Cat$PS_juv[1:10]) + sum(Cat$PS_spa[1:10]) ) 

HCat<-Cat[1,]
rownames(HCat) <- rownames(HCat0)
HCat[1,] <- c(0,0,prop_juv*HCat0$PS,(1-prop_juv)*HCat0$PS,HCat0$WEIR.SO)

# --- CAA data ------

CAA<-readRDS(file=paste0(datdir,"CAA_3_200228.rds")) # age, fleet, year (needs to be year, age, fleet)
...
CAL<-readRDS(file=paste0(datdir,"CAL_3_200402.rds")) # age, fleet, year (needs to be year, age, fleet)
...
#if(all(dimnames(CAL)[[1]]==dimnames(AS_CAL)[[1]])){ # check of compatible dimensions of reported CAL data for fishery and surveys
#  data$length_bin<-as.numeric(dimnames(CAL)[[1]])+0.5  # mid point is 5mm higher per bin
#}else{
#  message("CAL data are differing in dimensions among fleets / surveys")
#}
...
#G_ind<-(1:ny)[Year %in% Gpars$Q_Year] # by VB params
...
# by VB params
#Len_age[yamat]<-Gpars$Linf[yind]*(1-exp((-yamat[,2]-Gpars$t0[yind])*Gpars$K[yind]))
#Wt_age[yamat]<-1e-6*Gpars$a[yind]*(Gpars$Linf[yind]*(1-exp((-yamat[,2]-Gpars$t0[yind])*Gpars$K[yind])))^Gpars$b[yind]
...
# 0.2 all ages all years (ADAPT - VPA Model A, DFO 2011)
# 1=0.7, age 2=0.35, age 3+=0.27  (ADAPT - VPA Model B, DFO 2011)
...
#Mat <- readRDS(paste0(datdir,"MAT_200218.rds"))
...
OM <- DHOM(Name="OM_Fleet_3", nsim=nsim, ny=ny, na=na, np=50, h=0.95,
...
misc <- list(ESS=c(1000,1000), max_F=10, selectivity=c('dome','dome','dome','logistic','dome'),
          s_selectivity=c("dome"), f_name=names(Cat), s_name<-c("Aco. Surv.","Larval"),
          LWT=list(Index=rep(1,2), s_CAA = 0.05, s_CAL = 0, CAA = rep(0.05,5), CAL=rep(0,5)),

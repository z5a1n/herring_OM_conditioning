...
Pred<-read.csv("C:/Users/BARRETTTJ/Desktop/Herring paper/Time varying M/Species Biomass DFO survey/PredBiomass.csv")
...
# Matrix of starting values for fishery selectivity parameters: row 1 = LFS, row 2 = L5, row 3 = Vmaxlen (if dome)
vul_par <- map_vul_par <- matrix(NA, 3, ncol(Cat))
vul_par[, 1:4] <- c(OM@LFS[1], OM@L5[1], 1)
vul_par[, 5] <- c(2, 1, 1)

# The map matrix indicates fixed parameters (NA) or estimated parameters (integer) for the corresponding entry in vul_par
map_vul_par[1:3, 1:3] <- 1:9
map_vul_par[1:2, 4] <- 10:11

misc <- list(ESS=c(1000,1000), max_F=10, selectivity=c('dome','dome','dome','logistic','logistic'),
             s_selectivity=c("dome","dome"), f_name=names(Cat), s_name<-c("Aco. Surv.","Larval"),
             LWT=list(Index=rep(1,2), s_CAA = 0.05, s_CAL = 0, CAA = c(rep(0.05,4),0), CAL=rep(0,5)),
             comp_like="multinomial", vul_par = vul_par, map_vul_par = map_vul_par)#c(1,1,0.5,0.5,0.5)))
